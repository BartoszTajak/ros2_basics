...
class object_position(Node):
    def __init__(self):
        ...
        self.pub = self.create_publisher(Point, 'position', 10)  # typ, temat, queue depth
        self.timer = self.create_timer(1, self.tick)           # callback co 1 s
        self.i = 0
...
def main():
    rclpy.init()
    node = object_position()        # ← TERAZ `node` istnieje
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    finally:
        node.destroy_node()         # ← teraz to ma sens
        rclpy.shutdown()
